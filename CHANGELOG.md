...
- **"Model-tier discipline" section** added to all four root docs in `templates/agents/` (`AGENTS.md`, `AGENTS.codex.md`, `CLAUDE.md`, `copilot-instructions.md`), documenting the cheap-first rule of thumb, the **platform-specific delegate behaviour** (on Claude Code each delegate's own `model:` pin wins for that turn; on Copilot skills inherit the invoking prompt's model, so an inline skill from a smart-tier orchestrator runs smart — invoke the skill as a standalone chat to keep it cheap; on Codex the session runs one model set by `codex --model` and delegate `model:` is advisory-only), and the per-agent override surface (Copilot picker / Claude `/model` / Codex `--model`). `CLAUDE.md` also gets a **"When to escalate `sonnet` → `opus`"** paragraph explaining why `sonnet` is the default smart tier (matches or beats `opus` on `SWE-Bench Verified` at 5× lower cost) and when the escalation is actually justified.
...
- **`core-review` skill gains `model: haiku` frontmatter and `--budget {quick,medium,thorough}` argument**:
  ...
  Cheap tier is safe at every budget — `thorough` widens the file set, not the reasoning depth. **Escalating to a smart-tier review is platform-specific** because the shipped `model: haiku` pin is honoured differently per agent: on **Claude Code** the pin wins over `/model`, so escalation requires editing the installed `.agents/skills/core-review/SKILL.md` `model:` line (or setting `models.claude.cheap: sonnet` in `.agents-toolkit.json` and reinstalling, or reinstalling with `--model-pins off` so `/model` and the session default take effect); on **Copilot** the pin is advisory (skills inherit the invoking prompt) — escalation happens naturally when the caller is smart-pinned (`create-github-pr`, `resolve-github-reviews`), and standalone-chat invocations respect the picker; on **Codex** the session model set by `codex --model` applies. The skill never hard-codes a smart-tier override. The skill's "How to run it" section documents the per-platform escape hatches inline.
...
#### M5 — Measurement protocol (post-release, no code)

- **Inline definition** (the design plan doc was removed at PR creation; the protocol lives here instead of in a deleted file). Post-merge window: **2 heavy users × 5 working days**. Per-user/per-day metrics: input/output tokens, prompts by tier (cheap vs smart), and cheap-tier failure count (task had to be re-attempted on the smart tier). **Failure-driven demotion policy:** ≥ 2/5 failures on the same prompt class in the window ships a patch-release moving that prompt back to the smart tier. Measurement runs after this release ships; any resulting demotions land as follow-up patches referencing this changelog entry.
...
- **Documented the "subagent override filename must match `name:` frontmatter" rule** in the repo dev docs (`AGENTS.md`, `CLAUDE.md`) and inline in `templates/shared/agents/Explore.md`. Claude Code loads `.claude/agents/<name>.md` by filename stem, so renaming `Explore.md` to `EXPLORE.md` / `explore.md` (or any kebab-case variant) would register a *new* subagent instead of overriding the built-in `Explore` — silently leaving the built-in on its default (smart) tier and defeating the `haiku` pin. The rule is now called out as an explicit exception to the repo's kebab-case naming convention.
- **`resolve-github-reviews` header rewritten to reflect the pin-locks-invocation reality.** The previous header suggested per-run cheap-switch via picker / `/model` / frontmatter edit; Copilot correctly flagged that once the invocation starts, the `model:` pin is locked. New header lists three actually-executable paths: (a) edit + revert frontmatter, (b) reinstall with `--model-pins off`, (c) run the mechanical `gh`/GraphQL steps standalone from a cheap-tier chat and invoke the prompt only for the fix step.
- **`templates/agents/CLAUDE.md` "When to escalate `sonnet` → `opus`" section** — replaced the `"/model opus" one-off escalation` guidance (which cannot override a `model: sonnet` pin) with edit-installed-frontmatter-and-revert guidance. Same pin-locks-invocation reality as above.
- **`core-review` SKILL `--budget` callers table** narrowed to name only the actual auto-wired callers (`finalize-github-pr` for `quick`, `create-github-pr` for `medium`; `resolve-github-reviews` also on `quick`). Dropped the misleading `finalize-*` glob and `create-pr` reference — the Jira variants do not invoke `core-review`. Added a note directing standalone Jira callers to invoke `core-review` manually with an explicit `--budget`.
- **CHANGELOG M5 entry rewritten inline.** The previous entry referenced "the plan doc §6" for the measurement protocol, but the design plan doc was removed at PR creation, leaving a dead reference. The protocol (2 heavy users × 5 working days, per-user/per-day metrics, ≥ 2/5 failure-driven demotion policy) now lives in the changelog entry itself.
- **`SKILL.md` `model:` frontmatter now honours `--model-pins off` and `models.claude.cheap`/`smart` overrides.** Previously `installSkillsStandard` copied skills verbatim through `copyDir` without any content transform, so the shipped `model: haiku` in `core-review/SKILL.md` (and any future pinned skill) ignored both the strip and the substitution paths — defeating the M4 override story for skills. New `transformSkillFrontmatter` helper rewrites/strips just the `model:` line while preserving `name`, `description`, `argument-hint`, `allowed-tools`, etc. Threaded through the 3 `installSkillsStandard` call sites (Copilot/Codex install, Claude install, `restore` — `restore` re-resolves settings from the config chain so it does not undo the transform). Backward-compatible: fast path skips the transform entirely when no override is active and no strip is requested. 2 new tests in `src/cli.test.js` (61 → 63 passing).
- **"Delegate's `model:` pin wins for that turn" claim qualified per-platform across every surface that repeated it.** The universal-invariant framing was only correct on Claude Code (where skills and slash-commands each establish their own `model:` boundary). On Copilot, skills inherit the invoking prompt's model — an inline `core-review` from a smart-tier orchestrator therefore runs on the smart tier too, not on the skill's `haiku` pin. On Codex, the entire session runs one model (`codex --model`), so no delegate can switch mid-session. Rewrote the relevant paragraphs in `templates/agents/AGENTS.md`, `templates/agents/copilot-instructions.md`, `templates/agents/AGENTS.codex.md`, and the `**Model:**` header of `templates/shared/prompts/create-github-pr.prompt.md` to state the per-platform reality (with the actual escape-hatch procedure per platform: standalone chat on Copilot; separate `codex --model <cheap>` session on Codex). Also updated the CHANGELOG entries earlier in this release that repeated the false universal invariant.
- **`core-review` skill: delegation brief, P2 example, and re-run instruction now defer to the caller's `--budget`** instead of hard-coding "whole repository". The Claude Code subagent brief now spells out the file set per budget (`quick` → diff + directly touched files; `medium` → diff + one-hop neighbours; `thorough` → whole repo); the P2 principle's example was updated so it does not contradict `--budget` (the constant is *scope-selection contract*, not scope itself, which is user-selected); and the "acting on findings" convergence step now re-runs at the same `--budget` rather than always over the whole repo, with an explicit escalation clause when a fix ripples beyond the initial scope.
- **`core-review` "How to run it" and "cheap-tier by default" claims corrected for Copilot.** The previous text listed `finalize-github-pr` and `resolve-github-reviews` together as "callers that pin cheap", but `resolve-github-reviews` is smart-pinned — so on Copilot, its inline `core-review` pass runs on the smart tier (Copilot skills inherit the invoking prompt's model). Rewrote both paragraphs to state the per-platform reality: on Claude Code the `model: haiku` pin actually forces cheap; on Copilot the pin is advisory and holds only under cheap-pinned callers (`finalize-github-pr`) or standalone invocations with a cheap picker; on Codex the session model applies. Same qualification propagated to `templates/shared/prompts/README.md` (both the three misleading table-row parentheticals — `work-ticket`/`work-github-issue`, `create-pr`/`create-github-pr`, and `resolve-github-reviews` — and the "Autonomous cycles" paragraph, which now mirrors the three-bullet per-platform block from `templates/agents/AGENTS.md`).
- **CHANGELOG M4 `core-review` release-note override list rewritten with executable per-platform escape hatches.** Previous text said "callers who genuinely need reasoning override at the agent level (Copilot picker, Claude `/model`, Codex `--model`)", but Claude `/model` cannot escalate a `model: haiku`-pinned skill (the pin wins over `/model`). Rewrote the paragraph to name the actually-executable escape hatches per platform: on Claude Code edit the installed `SKILL.md` `model:` line, or set `models.claude.cheap: sonnet` in `.agents-toolkit.json` and reinstall, or reinstall with `--model-pins off`; on Copilot escalation happens naturally with smart-pinned callers or via the picker on standalone invocations; on Codex `codex --model` sets the session-wide model.
- **`core-review/SKILL.md` gains an inline explanatory comment about the `model: haiku` pin and the expected VS Code Copilot linter warning.** VS Code's `chat-customizations-evaluations` extension flags `model:` as an unsupported skill attribute (its allow-list is `argument-hint`, `compatibility`, `context`, `description`, `disable-model-invocation`, `license`, `metadata`, `name`, `user-invocable`) and surfaces the warning in the Problems panel. The pin is deliberately kept because **Claude Code honours it per-turn** (the source of the M4 cost savings on the cycle's most frequent delegate) and Copilot's runtime silently ignores unknown skill attributes anyway. A collapsed HTML comment right after the frontmatter documents this trade-off so users encountering the warning see immediately that it is cosmetic and expected.
...
- **Autonomous-cycle invariant — platform-specific.** Orchestrators do not force their tier onto delegated steps **on Claude Code**, where each delegate has its own `model:` boundary (skills via `SKILL.md`, slash-commands via `.md` frontmatter) and the delegate's pin wins for that turn. On **Copilot** skills inherit the invoking prompt's model, so an inline delegate stays on the orchestrator's tier — users who want a cheap-tier delegate must invoke it as a standalone chat. On **Codex** the session runs one model (`codex --model`) and delegates cannot switch mid-session; separate sessions are the only mechanism. The `--budget` on `core-review` scopes the file set, not the model — that scoping is uniform across agents.
- **15 new tests** in `src/cli.test.js` (48 → 63 passing) covering the frontmatter transforms, `--model-pins`, config overrides, `Explore` install/skip, `--budget` wiring, and skill-frontmatter transform (`--model-pins off` strip + `models.claude.cheap` substitution on installed `SKILL.md`).
